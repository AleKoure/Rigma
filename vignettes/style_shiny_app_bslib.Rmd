...
rlang::fn_fmls_names(bslib::bs_theme) %>%
  tail(-3)
```
By using these names into a 'Figma' file we can directly manipulate the theme 
of the Application by updating the 'Figma' file.
...
Using the `Rigma` package we can access published data via 'Figma' REST API.
...
The color is encoded in node ids and in thumbnail URLs provided by the API. We
include an example function that collects color data using the
thumbnail URLs. It will add an additional variable named as `color`.
...
Also, metadata about text can be extracted using the following function:
